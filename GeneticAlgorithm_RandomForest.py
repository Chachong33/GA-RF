...
# 根据参数生成随机森林，并且返回精度得分
def get_RandomForest(n_estimators_value,max_depth_value,max_features_value,train,test):
    train_y = train['VCI']
    train_x = train.values[:, :-1]
    test_x = test.values[:, :-1]
    test_y = test['VCI']
    rf = RandomForestClassifier(n_estimators=n_estimators_value, max_depth=max_depth_value,
                                max_features=max_features_value, n_jobs=-1)
...
def com_objvalue(pop,train,test):
    ...
    for i in range(len(values)):
        ...
        score = get_RandomForest(n_estimators_value,max_depth_value,max_features_value,train,test)
...
def cross_score(n_estimators,max_depth,max_features):
    ...
    X = data.values[:,:-1]
    Y = data['VCI']
    rfc = RandomForestClassifier(n_estimators=n_estimators,max_depth=max_depth,max_features=max_features, n_jobs=-1)
...
if __name__ == '__main__':
    ...
    for i in range(generations):
        ...
        print("best_individual: "+str(best_individual) + " " +"best_score: "+ str(best_fit)+" "+"average_score:"+str(average_score))
        # 自然选择
    ...
    print("最好结果：测试集评分：%s 交叉验证评分：%s 树的数量: %s 最大深度：%s 划分最大特征：%s"%(res[0],res[1],res[2],res[3],res[4]))
